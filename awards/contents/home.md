<p id="contact-info"></p>

* ✉️ [Jingxugu@smail.nju.edu.cn](mailto:jingxugu@smail.nju.edu.cn)

[Github](https://github.com/GrainRainy) | [知乎](https://www.zhihu.com/people/can-ruo-fan-xing-73-19)

A freshman of NanJing University, majoring in Intelligent Science and Technology (Artificial Intelligence). 

-------

#### Education

🕒 **2022.9 - 2025.6**  
📍 **Shijiazhuang No.2 Senior High School.**  
📝 **TOP 0.05% in College Entrance Examination.**

🕒 **2025.9 - 2029.6** *(Undergraduate)*  
📍 **Nanjing University.**  
🎓 **Major in Software Engineering.**

------

#### Research Interests

I’m actively looking for **Embodied AI**, **Robotic Learning**, and **Physical General-Purpose Agent** <u>Research Opportunities.</u> This includes:
* **Data-Efficient Robot Learning:** Training robust Vision-Language-Action (VLA) models and world models without massive compute or data requirements.
* **Real-time Edge Intelligence:** Optimizing and deploying large foundation models onto resource-constrained edge hardware (e.g. NVIDIA Orin NX) with low-latency control loops.
* **Closed-loop Perception & Manipulation:** Unifying multi-modal perception with robust whole-body control to enable robots to operate reliably in unstructured, real-world environments.

Feel free to <a href="#contact-info">Contact Me</a>!

-----
